...
```{r}
hist(x = as.numeric(df$Survived),
  xlab = "Survived (count)",
  main = "Histogram of Survived (Titanic)"
)
```
...
df_lattice <- df
# plot
lattice::histogram(~Survived | Pclass, groups = Sex, data = df_lattice)
```


#### Lattice Density Plot (might not be relevant for logistic regression, i.e. logical outcome)

```{r}
# lattice::densityplot(~Survived | Pclass, 
#                      data = df_lattice, 
#                      groups = Pclass, 
#                      plot.points = FALSE, 
#                      auto.key = TRUE)
# TODO learn to understand and improve lattice plots
```
